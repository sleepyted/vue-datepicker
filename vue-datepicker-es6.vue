<template>
  <div
    class="cov-vue-date"
    :class="option.wrapperClass ? option.wrapperClass : {}"
    @touchstart="mxf_stopDefault"
    @touchmove="mxf_stopDefault"
    @touchend="mxf_stopDefault"
  >
    <div class="datepickbox">
      <!-- <input type="text" t?itle="input date" class="cov-datepicker" readonly="readonly" :placeholder="option.placeholder" v-model="date.time" :required="required" @click="showCheck" @focus="showCheck" :style="option.inputStyle ? option.inputStyle : {}" :class="option.inputClass ? option.inputClass : {}" ref="pickerInput"/> -->
      <span
        class="cov-datepicker"
        v-text="date.time || option.placeholder"
        @click="showCheck"
    ...
    <div
      ...
      @click="dismiss($event)"
      v-bind:style="{
        background: option.overlayOpacity
          ? 'rgba(0,0,0,' + option.overlayOpacity + ')'
          : 'rgba(0,0,0,0.5)'
      }"
    >
      <div
      ...
      >
        ...
        <div class="cov-date-box" v-if="showInfo.day">
          <div class="cov-picker-box">
            ...
            <div
              ...
              :style="
                day.checked
                  ? option.color && option.color.checkedDay
        ...
        <div class="cov-date-box list-box" v-if="showInfo.year">
          <div class="cov-picker-box date-list" id="yearList">
            <div
              class="date-item"
              v-for="(yearItem, index) in library.year"
              :key="index"
        ...
        <div class="cov-date-box list-box" v-if="showInfo.month">
          <div class="cov-picker-box date-list">
            <div
              class="date-item"
              v-for="(monthItem, index) in library.month"
              :key="index"
        ...
        <div class="button-box">
          <span @click="showInfo.check = false">{{
            option.buttons ? option.buttons.cancel : "Cancel"
          }}</span>
          <span @click="picked">{{
            option.buttons ? option.buttons.ok : "Ok"
          }}</span>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    showDay(time) {
      ...
      this.displayInfo.month = this.library.month[
        moment(this.checked.currentMoment).month()
      ];
      let days = [];
